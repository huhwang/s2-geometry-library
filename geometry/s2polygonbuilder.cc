...
S2Loop* S2PolygonBuilder::AssembleLoop(S2Point const& v0, S2Point const& v1,
    ...
    while (path.size() >= 2) {
        ...
        if (candidates != edges_->end()) {
            VertexSet const& vset = candidates->second;
            for (VertexSet::const_iterator i = vset.begin(); i != vset.end();
                 ++i) {
                // We prefer the leftmost outgoing edge, ignoring any reverse
                // edges.
                if (*i == v0) continue;
        ...
        if (!v2_found) {
            ...
            // In the case of undirected edges, we may have assembled a
            // clockwise
            // loop while trying to assemble a CCW loop.  To fix this, we
            // assemble
            // a new loop starting with an arbitrary edge in the reverse
            // direction.
            // This is guaranteed to assemble a loop that is interior to the
            // previous
            ...
            if (options_.validate() && !loop->IsValid()) {
                ...
                // happen if there is bad input data.  Throw away the whole
                // loop.
...
class S2PolygonBuilder::PointIndex {
   ...
   public:
    ...
    void QueryCap(S2Point const& axis, vector<S2Point>* output) {
        ...
        for (Map::const_iterator i = map_.lower_bound(id); i->first == id;
             ++i) {
    ...
    bool FindNearbyPoint(S2Point const& v0, S2Point const& v1,
        ...
        for (int i = ids_.size(); --i >= 0;) {
            ...
            for (Map::const_iterator j = map_.lower_bound(ids_[i].range_min());
                ...
                double dist =
                    S2EdgeUtil::GetDistance(p, v0, v1, normal).radians();
                if (dist < best_dist) {
                    best_dist = dist;
                    *nearby = p;
                }
